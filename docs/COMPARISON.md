...
**claude-self-reflect** — Elegant: a single ~45 MB Rust binary, local `fastembed`, no Docker, MIT. But it is fundamentally a *Claude-Code-private recall* tool — it indexes Claude Code's own transcripts and exposes them over stdio MCP only; there is no HTTP, no neutral write path for other agents. It also uses the **same embedding model as memhub** (all-MiniLM-L6-v2 / 384), so the two could interoperate later. memhub's difference: a neutral REST + MCP service multiple agents share, plus true structured LLM extraction (self-reflect's enrichment is a paid add-on).
